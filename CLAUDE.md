...
| Tests (JS: motor portado + seguridad + créditos) | `node test/verificar_nucleo.js && node test/verificar_escapar.js && node test/verificar_seguridad.mjs && node test/verificar_creditosia.mjs && node test/verificar_sin_innerhtml_crudo.mjs` (sin `npm install`, solo Node >= 18) |
